...
for i in range(len(speeds)):
    ...
    out_path = os.path.join(main_datapath,'Subj04','OS','mrs_TGCS_v2')
    tspan  = [20, 26]
    ...
    my_mrs.set_filename(base_filename + '_' + str(speeds[i]) + '_mrs_minact')
    my_mrs.formulate_solve_ocp(dt=0.01, tstart=tspan[0], tend=tspan[1],
                               objective_function='min_act')

    # my_mrs.set_filename(base_filename + '_' + str(speeds[i]) + '_mrs_minP')
    # my_mrs.formulate_solve_ocp(dt=0.01, tstart=tspan[0], tend=tspan[1],
    ...
    my_linact_solver.set_filename(base_filename + '_' + str(speeds[i]) + '_linact_minact')
    my_linact_solver.formulate_solve_ocp(dt=0.01, tstart=tspan[0], tend=tspan[1],
                                         objective_function='min_act')

    # my_linact_solver.set_filename(base_filename + '_' + str(speeds[i]) + '_linact_minP')
    # my_linact_solver.formulate_solve_ocp(dt=0.01, tstart=tspan[0], tend=tspan[1],
    #                                      objective_function='min_pos_fiber_power')
